...
class EntityAttribute():
    ...
    def __init__(self, _object, ipmd, concreteDataType=None, baseEntity=False, encode=False):
        ...
        # Check explicitly if Python 2 is used
            ...
            self.value = tempDict
        elif objectType is genpy.rostime.Time :
            self.type = "dateTime"
            timezoneInfo = datetime.datetime.now().astimezone().tzinfo
            dateTimeValue = datetime.datetime.fromtimestamp(_object.to_time(), tz=timezoneInfo)
            isoTimeStr = dateTimeValue.isoformat(timespec='milliseconds')
            # if not encode:
            #     self.value = isoTimeStr
            # else:
            #     self.value = quote.quote(isoTimeStr, safe='')
            self.value = isoTimeStr
            self.setConcreteMetaData("time")
            self.setPythonMetaData(ipmd, "unicode")
        else:
            # Case it is a Class

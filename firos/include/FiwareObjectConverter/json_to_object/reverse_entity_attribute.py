...
class ReverseEntityAttribute(object):
    ...
    def __init__(self, _dict, useMetaData=True, encoded=False):
        ...
        elif _dict['type'].lower() in TIME_TYPES:
            if encoded:
                self.value = quote.unquote(_dict['value'])

            dateTimeValue = None
            try:
                dateTimeValue = datetime.datetime.fromisoformat(self.value)
            except ValueError:
                # check if string contains 'Z' means UTC
                indexOfZIdentifier = str(self.value).rfind('Z')
                if -1 != indexOfZIdentifier:
                    self.value = str(self.value).replace('Z', '+00:00')
                    dateTimeValue = datetime.datetime.fromisoformat(self.value)
                else :
                    dateTimeValue = datetime.datetime.now()

            posixTimeStamp = dateTimeValue.timestamp()
            rostimeValue = genpy.rostime.Time.from_sec(posixTimeStamp)
            self.value = rostimeValue
